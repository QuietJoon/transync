...
| STUB-045 | `transync_openai::pagination::split_oversize`                  | (Removed 2026-08-05 — DCR-0019 **deleted** the whole `pagination` module, following STUB-029's consumer-side hook out the door for the same reason: an empty no-op with zero callers documents nothing. The deferral this note recorded resolved 2026-08-09 — DCR-0026, ticket `fc0304`, slices SL-100..SL-105 — in opposite directions for its two terms. Provider crates keep **no** batching logic of their own: the "oversize halving" STUB-045 registered is the row-window split that shipped core-side at packing time (DCR-0026), and its reactive form — re-scoping a unit after a provider oversize signal — is rejected, not deferred (ADR-0017). `unit::split::split_oversize_tables` replaces an oversize table with header-carrying row windows exactly once, before round one, and `pipeline::merge` merges them back before regen, so a window id never reaches regeneration, the alignment map or the DOM; the "re-yields when over budget" half of this row's exit condition is that reactive form, which DCR-0026's "Out of scope" list rejects on ADR-0017's owner decision of 2026-07-24 that a batch-terminal provider failure aborts the run with no per-batch fallback rung. Reopening it would take a new owner record over ADR-0017; if that ever happens it belongs alongside the retry/fallback policy module (`pipeline/policy.rs`), not a resurrected stub.) |
