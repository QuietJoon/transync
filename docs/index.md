...
- [ADR 0028 — The source-language gate is a pre-flight question, not a second detection](decisions/0028-the-language-gate-is-a-pre-flight-question.md) — `transync-lang` answers "should a run start?" for a caller before any run; the alignment map's `detected_source_language` answers "what language was this, per the model that translated it". Separated by construction — no language in the verdict, no serialization, no workspace-member dependency — because prose separation is what the four 2026-09-01 two-opinion defects each had. (Ticket `e4f4b0`.)
...
- [DCR-0041 — The element walk models HTML integration points and foreign-content breakout](project/design-change-records/DCR-0041-foreign-content-islands-and-breakout.md) — `walk_elements` carries a three-valued content mode per open element instead of one inherited bool, so `foreignObject`/`desc` and the MathML islands return their children to HTML and the breakout tag set pops foreign elements; closes a live `contracts.md` §4a break reachable from untrusted source Markdown (ticket `e77173`).
- [DCR-0042 — Raw text and RCDATA are HTML-content states, and the scanner owns the content-mode model](project/design-change-records/DCR-0042-raw-text-is-an-html-content-state.md) — `scan_tags` enters raw-text/RCDATA only in HTML content, so inside `svg`/`math` those four names are ordinary foreign elements; the content-mode stack moves from `walk_elements` into the scanner so both layers read one verdict, SVG `<title>` becomes the integration point the spec makes it, and a sync attribute planted there stops surviving the strip (ticket `2e2453`).
- [DCR-0043 — Voidness is an HTML-content rule, and the breakout model already retired the hazard that kept it global](project/design-change-records/DCR-0043-voidness-is-an-html-content-rule.md) — foreign content has no void elements, so `<svg><link>a</link>` opens and its author's end tag is a real closer instead of an orphan `balance_fragment` deletes; safe because the five void names that are also breakout tags leave foreign content before they are processed, which DCR-0041 established and nobody had noticed answered this (ticket `48f3c6`).
- [DCR-0044 — The alignment row decides whether a block gets a DOM anchor, and `sync_role_for` names every `BlockKind` variant](project/design-change-records/DCR-0044-the-row-decides-the-anchor.md) — the sync-attribute omission moves out of `render_block`'s literal `ThematicBreak` test into `write_attrs`, keyed on the row's own `sync_role`, so a `Title` block cannot render with an anchor its row denies; and `sync_role_for` drops its `_ => Anchor` catch-all so a new `BlockKind` cannot inherit a wire-visible role nobody chose (tickets `18b9c3`, `9ffb97`).
- [DCR-0045 — `transync-lang` joins the workspace as the source-language gate](project/design-change-records/DCR-0045-transync-lang-source-language-gate.md) — a ninth member answering whether a translation run should start at all: script test first, `whichlang` second, chosen by a benchmark that took two rounds and reversed its own first recommendation; it depends on no workspace member and its `Verdict` names no language, so it cannot become the pipeline's provider-authored `detected_source_language` (ticket `e4f4b0`).
- [DCR-0046 — An offline run against a warm cache is supported behind `--offline`](project/design-change-records/DCR-0046-offline-runs-from-a-warm-cache.md) — the provider was built before the cache was consulted, so a run whose every unit was a hit still demanded a key; `--offline` builds a credential-free `TransyncOpenAI` whose fingerprint is byte-identical to the credentialed one, because `CacheKey` namespaces on that fingerprint and a stand-in with its own would have missed every entry and read as a corrupt cache (ticket `30a744`, OI-0038).
- [DCR-0047 — `balance_fragment` repairs a fragment that ends inside an unterminated region](project/design-change-records/DCR-0047-the-balancer-repairs-an-unterminated-trailing-region.md) — three of the four unterminated region kinds end at the first `>`, which in a mounted pane is the sync wrapper's own `</div>`, so every following block nested inside this one's wrapper; `TagToken::Skip` now carries the region's kind and terminated-ness so the balancer repairs it without ever classifying bytes itself (ticket `c1f9a8`).
...
- [Git history loss (2026-08-17)](project/git-history-loss-2026-08-17.md) — why this repository's commit graph begins on 2026-08-17: the cause named with physical evidence (a file-sync client writing conflict copies inside `.git`), the exclusion that was written and did **not** hold — the loss recurred on 2026-08-19 with the ignore line in place — the recovery runbook that got 17 of 33 objects back, the 16 that were lost, where the archived object store lives, the 2026-09-01 owner decision that the authoritative `.git` is GitHub and what that obliges, and what to do when a commit hash quoted in a ticket, ADR, DCR or CHANGELOG entry does not resolve.
