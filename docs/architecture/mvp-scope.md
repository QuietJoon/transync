...
- `tiktoken-rs`-based token estimation (shipped). Provider crates keep **no** batching logic of their own: the "oversize halving" STUB-045 registered is the row-window split that shipped core-side at packing time (DCR-0026), and its reactive form — re-scoping a unit after a provider oversize signal — is rejected, not deferred (ADR-0017). That split landed **2026-08-09** (DCR-0026, ticket `fc0304`, slices SL-100..SL-105, carried by v0.4.0) in `transync-core` and not in `transync-openai`: `unit::split::split_oversize_tables` replaces an oversize table with header-carrying row windows exactly once, before round one, and `pipeline::merge` reassembles one table block before regeneration, so a window id never reaches `regen::regenerate`, the alignment map or the DOM; `[constraints].default_table_strategy` and `--table-strategy` select it. The rejection is DCR-0026's "Out of scope" entry resting on ADR-0017's owner decision of 2026-07-24 — a batch-terminal provider failure aborts the whole run, with deliberately no per-batch fallback rung — so no provider signal re-scopes a unit after packing. The `pagination::split_oversize` no-op stub was deleted 2026-08-05 (DCR-0019) after its consumer-side counterpart went in the v0.2.0 curation, and `contracts.md` §5 carries the shipped rule: "A row-window split is packing, not retrying".
