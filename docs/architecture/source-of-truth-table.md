...
| Per-unit output validation rules                 | `transync-core::validate`                           | Layered checks (schema → IDs → per-kind → fragment reparse → full document gate: full reparse for Markdown, full rescan for HTML, dispatched on Document.format in pipeline::finalize — DCR-0036). |
