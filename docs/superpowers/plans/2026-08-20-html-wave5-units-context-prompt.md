...
```
  ...
  - `docs/project/phase-state.yaml`: `project.last_updated` → `2026-08-20-ti490d97-wave5`; `design.closed_change_records` appends `- DCR-0037-html-translation-entry-units-context-prompt` at the list's indent; `project.notes` gains a plain-prose paragraph (no backticks, `->` for arrows, at the block's four-space indent) summarizing the wave in the shape the earlier waves used.
