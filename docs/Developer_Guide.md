...
└── scripts/
    ...
    ├── test-browser.sh       headless Playwright: SCN-13 + the wasm demo
...
| `./scripts/test-browser.sh`   | Headless Playwright suite: SCN-13 dual-pane sync (`web/tests/scn13.spec.js`) plus the wasm render+edit demo (`web/tests/wasm.spec.js`). Regenerates the CLI fixture and runs `build-wasm.sh`, so it inherits the same prerequisites plus Chromium. |
