# Changelog archive — v0.1.0 through v0.4.0

This is an **archive** (DCR-0054). It holds the released `CHANGELOG.md` sections
for v0.1.0, v0.2.0, v0.3.0 and v0.4.0, verbatim and unedited. A released
changelog section is a record: its words do not change, and nothing is appended
to it.

`CHANGELOG.md` keeps `[Unreleased]`, the current release, and a heading and
pointer for each release below, so its heading-to-link-definition invariant
(release-checklist step 16) still holds and every `[x.y.z]` link still resolves.

Read this for what shipped in a released version. For what is true now, read
`CHANGELOG.md` and `docs/architecture/contracts.md`.
