...
- **Layered validation**: schema → ID-set equality → per-kind shape (table cols + alignment + row count, list topology, code-fence info, heading level, blockquote children) → fragment reparse → visible-text presence (a translation that renders nothing where the source had visible text is rejected; DCR-0048) → inline protection (link/image destinations; policy-gated code spans) → full-document reparse. Any layer rejecting triggers a bounded verbatim resubmission of the failed units — same prompt, same scope (ADR-0009); persistent failure falls back to source bytes with `fallback_status: fallback_source` recorded on the alignment map. Never silent corruption.
...
- **Schema-versioned alignment map** (`schema_version: "1.3.0"`) — durable wire format between the renderer and the JS engine. Consumers MUST reject unknown majors.
...
```
   .md | .html  ──▶  parse/intake  ──▶  Document IR  ──▶  build batches  ──▶  Translator
                                                                         ...
                                                                         per-batch result
                                                                                   │
                                       regenerated document  ◀── regen ◀── validate (7 layers)
                                       │
                          ┌────────────┴────────────┐
                          ▼                         ▼
...
├── crates/
│   ├── transync-lang/        # source-language GATE — "should a run start at all?"; depends on
...
| Browser verification — SCN-13 dual-pane sync and the SCN-16 HTML-run bundle (Playwright suite is primary; manual checklist kept) | [`web/tests/`](web/tests/) via `scripts/test-browser.sh`; [`web/SMOKE.md`](web/SMOKE.md) |
