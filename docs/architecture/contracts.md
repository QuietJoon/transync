...
- **A failed append closes the log for the rest of the process, and that is consumer-visible (OI-0044 / R0009-0081, 2026-09-05).** `DiskState` holds the writer as an `Option`. The first write error `take()`s it and drops the buffered remainder rather than flushing it, warns once, and returns the original error; every later `put` or `evict` answers a `CacheError` naming the closed log, while `get` keeps serving the index it already replayed. This is the only shape in which a cache operation fails for a reason that is not the operation's own — so a consumer seeing repeated `CacheError`s after one failure is seeing this, not a new fault each time, and the `DiskCacheOptions` posture holds throughout: an accelerator must not kill a run, so the run degrades to re-translation rather than failing. Dropping the buffer instead of flushing it is what makes the damage recoverable: with nothing appended after them, the partially-written bytes are the file's *last* bytes, which is the torn tail replay already repairs, so the cost is the failed record and the one being written. Reopening is deliberately not attempted — an append-mode reopen positions at exactly the EOF those bytes end at and would weld identically.
...
- `provider_error` **narrows**: it still means "a provider failure", but only the *unclassified* one (`TranslatorError::Other`). Code that read `provider_error` as "any provider failure" now sees thirteen sibling codes it must treat as unrecognized-but-opaque, per the rule above. This narrowing is the one sanctioned exception to append-only, and it rides the 0.4.0 window.
...
- `NoProviderAvailable(String)` — this `Translator` has no provider to call, so the work it was just handed cannot be done by it at all. Raised by the CLI's `--offline` run when a unit misses the cache (ti `30a744`, DCR-0046); stable code `provider_unavailable`, exit `6`. It is its own variant rather than a message inside `Unsupported` because that code means the CLI cannot tell a configuration fault from a document one, while this is unambiguously the caller's configuration with the caller's remediation.
...
# Optional: the system prompt an HTML-document run compiles INSTEAD of
# `prompt` (ti `490d97` wave 5 / DCR-0037). Selected at the cohort-compile
# door by the run's source format; a profile without it falls back to
# `prompt` verbatim plus one advisory warning, never a synthesized merge.
prompt_html = """
...
- **A `non-sync` row carries `data-block-kind` and nothing else (ti `18b9c3`, 2026-09-01).** The sync set — `data-sync-id`, `data-order`, `data-fallback` — is omitted for *every* kind whose alignment row says `non-sync`, and the omission is decided **from the row's own `sync_role`**, not from a list of kinds kept beside it. Until this date it was a literal `BlockKind::ThematicBreak` test in `render_block`: correct for the only kind that could reach it, and silently wrong for the next one. `BlockKind::Title` is `non-sync` by decision D5, so it would have rendered with a real `data-sync-id` while its own row denied it — the row and the DOM disagreeing about one block, which is the thing every anchor rule in this section rests on not happening. Nothing minted a `Title` when this landed; the HTML intake (ADR-0025) does now, and the decision was discharged ahead of it rather than at it.
  ...
  - HTML `<title>`: not page content (invariant 1's visibility leg, ADR-0025), so no pane anchor. What the pane emits in its place has since been settled by the HTML render path: an HTML-source pane emits **no element at all** for a non-sync row (see *HTML-source panes* below). On the Markdown path it degrades to its own escaped bytes inside a `<div data-block-kind="title">`.
...
**The direct-child rule is a claim about a hand-written HTML parser, and §4b below says exactly how much of HTML's tree construction that parser implements** — including the residual that still breaks this rule on 8 of 10,000 generated fragments, and the browser oracle that measures it (DCR-0051).
...
**It is probed, not merely stated (R0003-0076).** A precondition whose violation is "a silent layout regression" cannot be left to documentation alone, so `mountSync` probes it once per pane at mount time and `console.warn`s on violation — naming the element the offsets are actually being measured against. **What it reads changed 2026-08-26 (R0009-0024, commit `69701cd`):** the probe used to read `offsetParent` on one representative anchor, which answers a question about *that anchor* rather than the precondition. The precondition is a fact about the **pane** — that it is positioned — so the probe now reads `getComputedStyle(pane).position` and returns when it is non-static. `offsetParent` survives on the failure path only, where naming the ancestor the offsets are really measured against is exactly what makes the warning actionable. **The budget widened 2026-09-04 (OI-0047), and the widening is the contract now:** the probe runs at mount *and* on every coalesced reflow recompute, so it is one property read per pane **per recompute frame** rather than one per pane at mount. The reason is that the old budget bought its cheapness by going silent exactly when geometry changes — a pane that becomes unpositioned *after* mount, or a map row whose anchor disappears on reflow, produced no diagnostic at all. The recompute is already coalesced (it is not per animation frame), and both diagnostics are **latched**: the verdict is remembered and a warning is emitted only when it changes, so a persistently-broken pane warns once rather than on every reflow. `warnMapDomDrift` rides the same latch. **Residual, recorded rather than closed:** a positioned wrapper around a *later* anchor, while the pane itself is correct, is still undetected — that is a different property from this precondition, and widening the probe to every anchor would break the per-mount budget above, so it is a contract change rather than a fix. It is a warning rather than a refusal: the geometry is wrong, but the panes still pair by block id and the caller may be mid-layout. **What the source pane measures, stated because a reader would otherwise infer more (OI-0048 / R0009-0078):** it measures the **Block's** `source_range`, never the alignment row's copy of it. The row's copy is equal by construction — `align::…` is the only production site that writes it — so the two cannot disagree in output this engine produced, and `PaneCtx` deliberately does not re-validate it. A consumer synthesizing its own map must therefore keep the row's `source_range` equal to the block's; the pane will not catch a disagreement, and that is a documented boundary rather than a missing check. A `null` `offsetParent` (`display: none`, a `position: fixed` subtree, a detached pane) is not the violation and is not warned about — there is no offset geometry to be wrong.
...
**Past the last claimed anchor the follower clamps rather than freezes (OI-0047, 2026-09-04; stated here 2026-09-05 after ti `8cd7ba`).** The reference engine's active-block scan answers with the **last-ending claimed anchor at `progress: 1`** once the reference line passes that anchor's bottom, mirroring the clamp it has always had above the first anchor. Until that date it answered "no active block" and the follower simply stopped, so a pane carrying more than a viewport of unanchored content below its last anchor — inside the scroll box — left the follower wherever it happened to be. The change is **consumer-visible**: a reader scrolling into that tail now MOVES the follower, to the partner's copy of that same anchor. It is not a route by which unclaimed content drives sync, and the distinction is worth stating because it has already been misread once. Content below the last anchor is what makes the tail reachable, and some of that content may carry a `data-sync-id` no row claims; such elements are already absent from the scan, since the row gate above runs at mount **and** on every reflow recompute, so both the id the clamp returns and the partner geometry the follower is driven to belong to a claimed anchor. An unlisted anchor buys nothing an ordinary unanchored paragraph of the same height does not: both extend the scroll box, neither can be selected. `web/tests/engine.spec.js` test `l` pins the clamp's destination out of the pane's own geometry, and pins both halves of the duplicate policy — the partner lookup and the scan array — across a reflow recompute, which is the only place the scan-array half is observable.
...
### 4b. Tree construction: what the balancer's walk models, and what it does not

**Added 2026-09-05 (DCR-0051).** §4's balancing guarantee rests on one question — "which element does this closer close, and what is still open at the end?" — and `transync-html` answers it by hand. Until this date it answered it **twice**: `scan_tags` kept a content-mode stack and `walk_elements` kept an open-element stack, and the two popped differently from each other and from a browser. That is the single root cause behind five tickets, two of which put an attacker-chosen `data-sync-id` into a live DOM and thereby defeated the render-side strip §4a's OI-0035 closure depends on. There is now **one** stack: `scan_tags_with_state` keeps it, every mutation is reported on the token, and `walk_elements` replays it and adds only extents, close spans and orphan spans.

**What is modelled**, each verified against headless Chromium through the same `innerHTML` fragment parse a pane is mounted with, not against the crate's own answers:

- In-body's **"any other end tag"** stops at the first **special** element instead of at the nearest name match. The category is namespace-qualified — MathML's `mi`/`mo`/`mn`/`ms`/`mtext`/`annotation-xml` and SVG's `foreignObject`/`desc`/`title` are special too, and are the same names that terminate a foreign scope search.
- HTML's **four scopes** — normal, list-item, button, table — for the block-level end tags, `li`, `h1`–`h6` and the table ones. Table scope is what makes `<div><table>a</div>` leave the table open: `table` terminates the search, a browser ignores the closer, and the crate used to pop the table and let the pane's own `</div>` be ignored the same way.
- **Foreign content's end-tag dispatch**, in the spec's step order: `</p>` and `</br>` are BREAKOUT end tags, and every other end tag name-tests the current node and each *foreign* node below it, checking the namespace first on every step after the first.
- The **start tags "in body" ignores** — `caption`, `col`, `colgroup`, `frame`, `tbody`, `td`, `tfoot`, `th`, `thead`, `tr` outside a table open no element and earn no closer.
- HTML's **implied end tags** applied by scope, in the one stack, so `<p><em><ul>` closes the paragraph through the `<em>`.
- The dispatcher's **`mglyph` / `malignmark`** exception inside MathML text integration points, and its `annotation-xml` + `<svg>` sibling.

**What is NOT modelled, and the direction each omission errs in.** Every one of them errs toward holding a frame a browser has discarded, which makes the balancer append a closer rather than lose one — the safe direction for §4a.

- **The adoption agency algorithm and the list of active formatting elements.** A formatting end tag goes through "any other end tag" instead. The appended redundant closer is a no-op in a browser, but *reconstruction* is not covered by that argument: a browser rebuilds a `<b>` when it inserts the next element, and that frame can stop an appended closer. Measured residual: 8 of 10,000 generated fragments still break §4a this way, down from 1,358. Owned by ti `525bef`.
- **Insertion modes.** `head`, `body`, `html` and `frameset` are on the same in-body ignore list as the table-section start tags and are deliberately kept, because `intake::html` walks whole HTML documents where those four really do open elements; `</body>` and `</html>` are treated as ordinary scope-based end tags; and "clear the stack back to a table context" is not modelled.
- **Foster parenting.** A browser's insertion point inside a table is before the table; the crate's is inside it. This changes where the *next* byte lands, not whether the wrapper survives.

**"Orphan close tags are dropped" is narrower than it was, and the line is the pane's.** A fragment is mounted inside the sync wrapper's own `<div>`, so a browser's search for a closer's target continues past the bottom of the fragment's stack and into that wrapper. A closer whose search **runs off that bottom** is an orphan and is deleted, because leaving it lets `<svg><g></div>` close the wrapper (measured). A closer **stopped inside the fragment** by a special element or a scope terminator is inert — a browser stops at the same frame, above anything the wrapper contributes — and its bytes stay, because deleting on an approximation of a browser's verdict is not the same as deleting on the verdict. `</p>` and `</br>` are never orphans at all: a browser turns each into content (`<p>a<div>b</div>c</p>` renders `<p>a</p><div>b</div>c<p></p>`), so deleting one removes something the author's own bytes render.

The instrument for all of this is `web/tests/html-oracle.spec.js` (ti `ec235f`), which puts Chromium in the loop as an oracle not derived from the functions under test, and pins the residual as an exact census. On the alphabet DCR-0050 blessed and DCR-0051 did not change: 8,619 of 10,000 generated fragments in full agreement, 3,863 browser-declared abstentions, 9 divergence mechanisms, `sec4a:break-nested` 8, `reserved:dom-only` **0**.
...
- **Content types come from a fixed extension table, never from content sniffing** (`serve_cmd::mime`). It covers the bundle's file set (`.html` — and its `.htm` spelling, which shares the arm — `.json`, `.js`) and what `scripts/test-browser.sh` lays on top of it for the wasm demo (`.mjs`, `.wasm`, `.md`), plus `.css`, `.png`, `.svg`, `.txt` and `.map`. Everything else is `application/octet-stream`. Every response also carries `X-Content-Type-Options: nosniff` and `Cache-Control: no-store` — the latter so a regenerated bundle never loses to a cached `sync.js`.
...
- `6` — the provider refused the run because of **how it was configured**; change the configuration and re-run (ti `e62b59`, v0.4.0). Reached from `TranslatorError::Authentication` (the credential), `ProviderRejected` (the request itself was refused — `status: Some(404)` is a `--model` that does not exist), `OutputCeilingExhausted` (`[batching].target_output_tokens`) `ContextWindowExceeded` (the `[batching]` input budget and `max_units_per_batch`) and `NoProviderAvailable` (`--offline` — the run was configured with no provider and then reached work only a provider could do; drop the flag, or warm the cache it was pointed at; ti `30a744`). Five causes and five different knobs, but **one action**, which is the granularity a process exit code can carry. Deliberately not `1`: that code means the CLI refused the arguments before anything ran, while this one means they were accepted, the run started, and the *provider* refused it — a caller that fixes its own argv and retries must not be told those are the same event.
...
impl TransyncOpenAI {
    ...
    // Credential-free (DCR-0046): validates the two namespace-bearing axes and
    // fingerprints IDENTICALLY to a credentialed instance built the same way.
...
therefore raised by `try_new`, `from_env`, `offline` and `ModelId::parse`, never by
`new`. `try_new` is the only constructor that inspects all **three** axes;
`offline` runs the same model-id and base-URL validation and skips only the
credential check it has no credential to run (DCR-0046).
...
**The value is taken verbatim; there is no domain check (R0011-0075, ADR-0031).**
`with_timeout` is infallible by design — it returns `Self`, not `Result<Self>` —
and stores whatever `Duration` the host hands it, with no floor and no clamp.
`Duration::ZERO` is therefore accepted at construction and means every request
fails immediately with a timeout error, while `request_timeout()` still reports
zero. That is the point of the accessor: it tells a host what it *set*, not what
this crate would have preferred. Clamping would make it report a budget nobody
chose, and any floor would be arbitrary — an in-process or fake `Translator`
behind the same trait can legitimately answer in microseconds, and a
fault-injection harness may want a budget that always expires.
...
Also as in §7, **the value is taken verbatim** — the builder is
infallible, no floor or clamp is applied, and `Duration::ZERO` fails every
request immediately while `request_timeout()` still reports zero (R0011-0075,
ADR-0031).
