...
**Overlap with wave 4, which runs in parallel:** wave 4 owns `transync-core::validate` (the layer-6 twin `full_rescan_html`) and the one format branch in `pipeline::finalize`. This plan touches **no file in `transync-core` at all** — the "no core change" half of the wave's one-sentence claim is also what makes the parallelism safe. The two waves' only shared artifact is a DCR *number*, and the numbers are **reserved, not first-come** (corrected 2026-08-23): **wave 3 holds DCR-0035 and wave 4 holds DCR-0036**, regardless of landing order — wave 4's own Task 5 Step 1 says so explicitly ("wave 3 holds DCR-0035 — this wave takes DCR-0036 even if 0033 or 0035 is not on disk yet"), waves 5/6/7 hardcode 0037/0038/0039, wave 2's Task 8 warns that "the next free number" **is a race**, and this plan's own body hardcodes DCR-0035 in four places. The clause used to read "whichever lands second takes the next free number"; a wave-3 implementer obeying that after wave 4 landed would have taken 0037 and collided with wave 5. Wave 4 may hand-build `Document`s with `format == SourceFormat::Html` before this wave lands; nothing here invalidates them.
...
- **PRECONDITION (added 2026-08-24 by design review): the foreign-content breakout gap, ti `e77173`, must be discharged before this wave's corpus is blessed.** `walk_elements` models neither HTML's breakout tags nor its integration points, so `<div class="wrap"><svg><div>x</svg></div>` leaves the inner `div` open in a browser while the walk closes it at `</svg>`. This wave consumes the **same walk** through `element_extents`, and deviation 6 makes the extents' verdict binding on phrasing-run boundaries — so on breakout inputs this intake draws block boundaries a browser disagrees with, and Task 5's identity corpus and kind-sequence pins would be blessed over them. A later walk fix then re-derives block sets, **ids and section paths** for that page class, and ids are this project's only sync currency. The disposition is recorded in `docs/backlog.md` (`html-walk-foreign-content-breakout`): **fix it between wave 2 closing and this wave starting**, not accept it. If you reach this plan and the fix has not landed, **stop and say so** rather than blessing a corpus the fix will move.
