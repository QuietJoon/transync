...
/// Which blocks' own regenerated bytes are past the nesting ceiling.
///
/// A refusal on the whole document says only that *something* in it is too
/// deep, and that is not enough: [`ReparseFailure`] is what `finalize` turns
/// into per-block fallback, so it has to name offenders or nothing gets
/// rolled back. [`BlockOffsets`] already records where each block's
/// regenerated bytes are, so ask each block the same question the document
/// just failed.
///
/// When no single block is over the ceiling the depth is an artefact of the
/// **assembly** rather than of any one payload — blocks that each nest
/// legally can still concatenate into a document that does not. Every block
/// is implicated then, because an empty list would tell `finalize` to fall
/// back nothing at all and the document would be published as-is.
fn blocks_over_depth_ceiling(regenerated_md: &str, offsets: &BlockOffsets) -> Vec<BlockId> {
    let mut over: Vec<BlockId> = offsets
        .0
        .iter()
        .filter(|(_, range)| {
            regenerated_md
        ...
        .collect();
    if over.is_empty() {
        over = offsets.0.keys().cloned().collect();
    }
    // `BlockOffsets` is a HashMap, so without this the offender list would
    // differ run to run for the same input.
    ...
    let root = match crate::parser::guarded_parse(&arena, regenerated_md, &parser_opts) {
        ...
        Err(too_deep) => {
            return Err(ReparseFailure {
                reason: too_deep.to_string(),
                divergent_source_blocks: blocks_over_depth_ceiling(regenerated_md, offsets),
            });
        }
    };
...
mod tests {
    ...
    #[test]
    fn reparse_full_refuses_a_regenerated_document_past_the_nesting_ceiling() {
        ...
        assert!(err.reason.contains("nesting too deep"), "got: {err}");
        assert!(
            !err.divergent_source_blocks.is_empty(),
            "a refusal that names no block tells finalize to roll back nothing"
        );
    }

    /// The attribution is per block, not "blame everything": when one
    /// block's own regenerated bytes are what is too deep, only that block
    /// is named. The all-blocks answer is reserved for depth that no single
    /// payload accounts for.
    #[test]
    fn a_single_over_deep_block_is_the_only_one_attributed() {
        let doc = parse("alpha\n\nbravo\n").expect("source parses");
        ...
        let err = reparse_full(&doc, &regenerated, &offsets)
